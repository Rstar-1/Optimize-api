...
class OrderProducer {
  ...
  static async publishEvent(topic, event) {
    try {
      ...
      const message = {
        key: event.data?.order_id || event.order_id,
      ...
      logger.info(`Order event published to topic: ${topic}`, {
  ...
  static async publishBatch(topic, events) {
    try {
      ...
      logger.info(`Batch of ${events.length} order events published to topic: ${topic}`);
  ...
  static async publishWithRetry(topic, event, maxRetries = 3) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        lastError = error;
        logger.warn(`Order publish attempt ${attempt}/${maxRetries} failed for topic ${topic}`);
    ...
    logger.error(`Failed to publish order event after ${maxRetries} retries`, lastError);
    throw lastError;
  }

  /**
   * Publish transaction event (multiple topics)
   */
  static async publishTransaction(events) {
    const kafkaClient = getKafkaClient();
    ...
    for (const { topic, event } of events) {
      try {
        ...
        results.push({ topic, status: 'success', result });
      } catch (error) {
